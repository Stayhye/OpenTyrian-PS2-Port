...
const char *custom_data_dir = "mass0:/OPENTYRI/DATA";
...
#ifdef TARGET_PS2
		init_scr();
		scr_printf("PS2 ERROR: Data not found!\n");
		scr_printf("Looked in: %s\n", custom_data_dir);
		while(1) {} // halt so you can read it
#endif
	}
		dir = "";
...
FILE *dir_fopen( const char *dir, const char *file, const char *mode )
{
	...
	{
		char buf[256];
		sprintf(buf, "fopen: %s = %s", path, f ? "OK" : "FAIL");
		ps2_log(buf);
	}
#endif
...
long ftell_eof( FILE *f )
{
	...
	{
		char buf[80];
		sprintf(buf, "ftell_eof: initial pos=%ld", pos);
		ps2_log(buf);
	}
#endif
	...
	{
		char buf[80];
		sprintf(buf, "ftell_eof: size=%ld", size);
		ps2_log(buf);
	}
#endif
