...
void push_joysticks_as_keyboard( void )
{
...
#ifdef TARGET_PS2
	{
		...
		if (pj_log < 20) {
			char buf[80];
			sprintf(buf, "pjk: j0.input=%d j0.confirm=%d j0.cancel=%d hat0=%d", joystick[0].input_pressed, joystick[0].confirm, joystick[0].cancel, SDL_JoystickGetHat(joystick[0].handle,0));
			ps2_log(buf);
			pj_log++;
		}
	}
...
#ifdef TARGET_PS2
	{
		...
		if (log_count < 3) {
			for (int j = 0; j < joysticks; j++) {
				if (joystick[j].handle == NULL) continue;
				char buf[256];
				int axes = SDL_JoystickNumAxes(joystick[j].handle);
				int buttons = SDL_JoystickNumButtons(joystick[j].handle);
				int hats = SDL_JoystickNumHats(joystick[j].handle);
				sprintf(buf, "joy%d: axes=%d btns=%d hats=%d", j, axes, buttons, hats);
				ps2_log(buf);
				for (int a = 0; a < axes && a < 4; a++) {
					sprintf(buf, "  axis%d=%d", a, SDL_JoystickGetAxis(joystick[j].handle, a));
					ps2_log(buf);
				}
			}
			log_count++;
...
void init_joysticks( void )
{
	...
	{
		char buf[80];
		sprintf(buf, "joysticks detected: %d", joysticks);
		ps2_log(buf);
	}
#endif
	if (joysticks == 0)
